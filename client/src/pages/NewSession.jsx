...
export default function NewSession() {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await axios.post(`${API}/api/sessions`, { session_date: date, notes: notes || null, shots });
      navigate('/my-stats');
    } catch (err) {
  ...
  const courtData = {};
  Object.entries(zoneData).forEach(([id, v]) => {
    const attempts = (v.makes || 0) + (v.misses || 0);
    if (attempts > 0) courtData[id] = { attempts, makes: v.makes || 0 };
  });
          ...
          <div className="card">
            <div className="form-row">
              ...
              <div className="form-group" style={{ flex: 2 }}>
                ...
                <input type="text" value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g. Morning practice, worked on threes" />
              </div>
            </div>
          </div>
